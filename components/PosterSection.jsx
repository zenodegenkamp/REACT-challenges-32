 ...
 */
	...
	const filteredArray = festivalData.filter(item => item.date === date);
	
	const sortedAndFilteredArray = filteredArray.sort((a, b) => {
		return a.importance - b.importance
	})
	
	console.log(sortedAndFilteredArray)
	const festivalParagraph = sortedAndFilteredArray.map(band => {
		
		// Create the font style depending on the importance
		let fontStyle = ""
		if(band.importance === 4){
			fontStyle = "small"
		} else if(band.importance === 3){
			fontStyle = "medium"
		} else if(band.importance === 2){
			fontStyle = "big"
		} else if(band.importance === 1){
			fontStyle = "giant"
		}
		
		return (
			<p className={fontStyle}>{band.name}</p>
		)
	})
